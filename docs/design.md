...
| 41 | A period is deleted only after its bundle has been verified, and a `chain_checkpoints` row records where each chain stood | `check_runs` grows forever and a plain `DELETE` leaves a gap that `sonde verify` correctly reports as tampering — indistinguishable, to a later reader, from someone removing a failing check. So: export, verify with the same command an auditor would run, record the boundary hash, delete — the checkpoint and the delete in one transaction, and nothing deleted if the bundle does not verify. The checkpoint is what lets the *next* bundle say what its first run follows once the rows before it are gone. Partitioning by month is the remaining optimisation and is a performance decision, not a correctness one. |
| 42 | Retiring a probe sets `revoked_at`; the row is never deleted | Its results are evidence and the chain that links them names the probe that signed it, so deleting the probe would leave a bundle nobody can verify and an auditor with a gap where a decommissioned cluster should be. What is withdrawn is the ability to act: the certificate stops authenticating, the scheduler skips it, and its queued work moves to a live probe. The dashboard shows it as retired rather than quiet, because "this cluster was decommissioned" is a different answer from "this probe stopped reporting". |
